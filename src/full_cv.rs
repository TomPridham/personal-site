...
pub fn full_cv() -> Result<Markup, Box<dyn Error>> {
    let other_cv_file = read_to_string(Path::new("src/other_cv.json"))?;
    let cv_file = read_to_string(Path::new("src/software_eng_cv.json"))?;
    let cv_items: Cv = serde_json::from_str(cv_file.as_str())?;
    let other_cv_items: Cv = serde_json::from_str(other_cv_file.as_str())?;
    let jobs = [cv_items.jobs, other_cv_items.jobs].concat();
    let skills = [cv_items.skills, other_cv_items.skills].concat();
    let volunteering_experience = other_cv_items.volunteering;
    ...
    let cv_html = html! {
        ...
        div.bottom-spacer{
            ...
            div{
                @for cv_item in jobs{
                    @if !cv_item.hide{
                        div.bottom-spacer.hr.job.other_job{
